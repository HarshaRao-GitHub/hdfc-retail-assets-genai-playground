...
export default function DownloadMenu({ content, filenamePrefix }: DownloadMenuProps) {
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    e.target.value = '';
    setLoading(true);
    try { await downloadInFormat(content, filenamePrefix, format); } catch { /* ignore */ } finally { setLoading(false); }
  };
    ...
    <div className="flex items-center gap-1.5">
      ...
      >
        ...
        {FORMAT_OPTIONS.map((opt) => (
          <option key={opt.format} value={opt.format}>{opt.icon} .{opt.format} — {opt.label}</option>
        ))}
      </select>
    </div>
  );
