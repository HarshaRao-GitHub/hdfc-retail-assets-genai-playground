...
function loadMermaid(): Promise<void> {
  ...
  mermaidPromise = new Promise<void>((resolve) => {
    ...
    script.onload = () => {
      // @ts-expect-error mermaid loaded via CDN
      window.mermaid?.initialize({ startOnLoad: false, theme: 'default', securityLevel: 'loose', suppressErrorRendering: true });
      mermaidReady = true;
      resolve();
    };
...
function MermaidBlock({ code }: { code: string }) {
  ...
  const render = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const mermaid = window.mermaid;
        if (mermaid) {
          const simplified = sanitized.replace(/quadrantChart/g, 'graph TD').replace(/xychart-beta/g, 'pie');
  ...
  return (
    <div ref={containerRef} className="my-3 p-4 bg-white border border-gray-200 rounded-lg overflow-x-auto flex justify-center" dangerouslySetInnerHTML={{ __html: svg }} />
  );
}
...
interface MarkdownProps { children: string; isStreaming?: boolean; }
...
function MarkdownInner({ children, isStreaming = false }: MarkdownProps) {
  const components = useMemo(() => ({
    code({ className, children: codeChildren, ...props }: React.ComponentPropsWithoutRef<'code'> & { className?: string }) {
      ...
      const lang = match?.[1];
      if (lang === 'mermaid') {
        if (isStreaming) return <MermaidPlaceholder />;
