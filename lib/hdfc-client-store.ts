...
const KEYS = {
  STAGE_RESULTS: 'hdfc_stage_results',
  APPROVALS: 'hdfc_approvals',
  SKIPPED_STAGES: 'hdfc_skipped_stages',
  FLOW_ID: 'hdfc_flow_id',
  FLOW_CREATED: 'hdfc_flow_created',
} as const;
...
export function ensureFlow(): { id: string; createdAt: string } {
  ...
  const id = `hdfc-flow-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
export function updateApproval(id: string, updates: Partial<ApprovalRecord>): void {
  const approvals = getApprovals();
...
export function getLatestApprovalForStage(slug: string): ApprovalRecord | null {
...
export function computeGateStatuses(): Record<string, GateStatus> {
  ...
  for (let i = 0; i < STAGE_ORDER.length; i++) {
    ...
    if (approval) {
      if (approval.status === 'pending') { result[slug] = 'awaiting_approval'; continue; }
      if (approval.status === 'approved' || approval.status === 'modified') { result[slug] = 'approved'; continue; }
      if (approval.status === 'rejected') { result[slug] = 'rejected'; continue; }
    }

    const stageResult = getStageResult(slug);
    if (stageResult) { result[slug] = 'awaiting_approval'; continue; }

    if (i === 0) { result[slug] = 'available'; continue; }
...
export function getUpstreamResults(slug: string): { slug: string; stageNumber: number; agentOutput: string }[] {
  ...
  for (let i = 0; i < idx; i++) {
    ...
    results.push({ slug: upSlug, stageNumber: sr.stageNumber, agentOutput: assistantMsgs[0] });
  }
  return results;
}
