...
function extractTablesAsCsv(md: string): string {
  ...
  while ((match = tableRegex.exec(md)) !== null) {
    ...
    tables.push(parseRow(header).join(','));
    body.split('\n').forEach(row => tables.push(parseRow(row).join(',')));
  }
...
function markdownToHtml(md: string): string {
  ...
  html = html.replace(/^(\|.+\|)\n(\|[\s:|-]+\|)\n((?:\|.+\|\n?)*)/gm, (_m, header: string, _sep, body: string) => {
    const thCells = header.split('|').filter((c: string) => c.trim()).map((c: string) =>
      `<th style="border:1px solid #d1d5db;padding:6px 10px;background:#004B87;color:white;font-size:11px;text-align:left;white-space:nowrap;">${c.trim()}</th>`
    ).join('');
  ...
  html = html.replace(/^## (.+)$/gm, '<h2 style="font-size:16px;font-weight:700;margin:20px 0 10px;color:#004B87;border-bottom:2px solid #004B87;padding-bottom:4px;">$1</h2>');
  html = html.replace(/^# (.+)$/gm, '<h1 style="font-size:20px;font-weight:800;margin:24px 0 12px;color:#004B87;">$1</h1>');
  html = html.replace(/\*\*\*(.+?)\*\*\*/g, '<strong><em>$1</em></strong>');
  html = html.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
  html = html.replace(/\*(.+?)\*/g, '<em>$1</em>');
  html = html.replace(/^>\s*(.+)$/gm, '<blockquote style="border-left:3px solid #004B87;padding:6px 12px;margin:8px 0;background:#eff6ff;color:#1e40af;font-size:12px;font-style:italic;">$1</blockquote>');
  html = html.replace(/^(\s*)[-*]\s+(.+)$/gm, (_m, indent: string, text: string) => {
    const depth = Math.floor(indent.length / 2);
    return `<div style="padding-left:${16 + depth * 16}px;margin:3px 0;font-size:12px;"><span style="color:#004B87;margin-right:6px;">&bull;</span>${text}</div>`;
  });
  html = html.replace(/^(\s*)(\d+)\.\s+(.+)$/gm, (_m, indent: string, num: string, text: string) => {
    const depth = Math.floor(indent.length / 2);
    return `<div style="padding-left:${16 + depth * 16}px;margin:3px 0;font-size:12px;"><span style="color:#004B87;font-weight:600;margin-right:6px;">${num}.</span>${text}</div>`;
  });
...
<div style="display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid #004B87;padding-bottom:10px;margin-bottom:16px;">
<div><div style="font-size:18px;font-weight:800;color:#004B87;">HDFC Retail Assets — GenAI Intelligence</div>
<div style="font-size:10px;color:#64748b;margin-top:2px;">Leadership GenAI Playground</div></div>
...
HDFC Retail Assets GenAI Platform &mdash; Auto-generated report &mdash; ${new Date().toLocaleDateString()}</div>
</body></html>`;
}
...
async function downloadPdf(content: string, filenamePrefix: string): Promise<void> {
  ...
  const styledHtml = buildStyledHtmlDocument(content, 'HDFC GenAI Report');
...
export async function downloadInFormat(content: string, filenamePrefix: string, format: ExportFormat): Promise<void> {
  ...
  switch (format) {
    case 'txt':
      triggerDownload(new Blob([stripMarkdown(content)], { type: 'text/plain;charset=utf-8' }), `${base}.txt`);
      break;
    case 'md':
      triggerDownload(new Blob([content], { type: 'text/markdown;charset=utf-8' }), `${base}.md`);
      break;
    case 'csv':
      triggerDownload(new Blob([extractTablesAsCsv(content)], { type: 'text/csv;charset=utf-8' }), `${base}.csv`);
      break;
    case 'xls':
      triggerDownload(new Blob([buildExcelHtml(content)], { type: 'application/vnd.ms-excel;charset=utf-8' }), `${base}.xls`);
      break;
    case 'pdf':
      await downloadPdf(content, filenamePrefix);
      break;
    case 'doc':
      triggerDownload(new Blob([buildStyledHtmlDocument(content, 'HDFC GenAI Report')], { type: 'application/msword;charset=utf-8' }), `${base}.doc`);
      break;
  }
}
