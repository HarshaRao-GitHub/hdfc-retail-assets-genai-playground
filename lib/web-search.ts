...
const NEEDS_SEARCH_PATTERNS = [
  ...
  /\b(regulation|policy|government|budget|act|bill|mandate|directive|RBI|SEBI|NHB)\b/i,
  /\b(compare|vs|versus|competitor|alternative)\b/i,
  /\b(market\s+size|market\s+growth|CAGR|forecast|projection|outlook)\b/i,
  /\bwho\s+is\b/i,
  /\b(home\s+loan\s+rate|repo\s+rate|interest\s+rate|PLR|MCLR|RLLR)\b/i,
  /\b(PMAY|pradhan\s+mantri|affordable\s+housing|CLSS)\b/i,
...
export async function searchWeb(
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === 'AbortError') {
      ...
      console.warn('Web search failed:', err instanceof Error ? err.message : 'unknown');
    }
    return [];
  }
}
...
export function formatSearchContext(results: SearchResult[]): string {
  ...
  return [
    ...
    '--- END WEB SEARCH ---',
    '',
    'Use the above web search results to supplement your knowledge with the latest information. Cite sources by number [1], [2], etc. when using web data.',
  ].join('\n');
}
