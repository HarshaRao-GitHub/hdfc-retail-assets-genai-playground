...
async function extractPDF(file: File): Promise<{ text: string; images: ImageAttachment[] }> {
  ...
  if (!extractedText) {
    extractedText = `[PDF: ${file.name}]\n\nText extraction was limited. File size: ${(buffer.length / 1024).toFixed(1)} KB.`;
  }
...
function extractTextFromPDFRaw(bytes: Buffer, filename: string): string {
  ...
  return `[PDF: ${filename}]\n\nThis PDF could not be fully parsed. File size: ${(bytes.length / 1024).toFixed(1)} KB.`;
}
...
function extractDocRaw(bytes: Buffer, filename: string): string {
  ...
  return `[DOC: ${filename}]\n\nThis document could not be fully parsed. File size: ${(bytes.length / 1024).toFixed(1)} KB.`;
}
